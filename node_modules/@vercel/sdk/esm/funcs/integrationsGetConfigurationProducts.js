...
import { GetConfigurationProductsRequest$outboundSchema, GetConfigurationProductsResponseBody$inboundSchema, } from "../models/getconfigurationproductsop.js";
 ...
 */
export function integrationsGetConfigurationProducts(client, request, options) {
    return new APIPromise($do(client, request, options));
}
async function $do(client, request, options) {
    const parsed = safeParse(request, (value) => GetConfigurationProductsRequest$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const path = pathToFunc("/v1/integrations/configuration/{id}/products")(pathParams);
    ...
    const [result] = await M.match(M.json(200, GetConfigurationProductsResponseBody$inboundSchema), M.fail([400, 401, 403, 404, "4XX"]), M.fail([500, "5XX"]))(response, req);
    if (!result.ok) {
        return [result, { status: "complete", request: req, response }];
    }
    return [result, { status: "complete", request: req, response }];
}
//# sourceMappingURL=integrationsGetConfigurationProducts.js.map