...
import { DangerouslyDeleteByTagsRequest$outboundSchema, } from "../models/dangerouslydeletebytagsop.js";
 ...
 */
export function edgeCacheDangerouslyDeleteByTags(client, request, options) {
    return new APIPromise($do(client, request, options));
}
async function $do(client, request, options) {
    const parsed = safeParse(request, (value) => DangerouslyDeleteByTagsRequest$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const [result] = await M.match(M.nil(200, z.void()), M.fail([400, 401, 403, 404, "4XX"]), M.fail("5XX"))(response, req);
    if (!result.ok) {
        return [result, { status: "complete", request: req, response }];
    }
    return [result, { status: "complete", request: req, response }];
}
//# sourceMappingURL=edgeCacheDangerouslyDeleteByTags.js.map